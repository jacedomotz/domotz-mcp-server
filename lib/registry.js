...
export async function dispatch(actionMap, actionName, args, domotzApi) {
  ...
  if (method === 'head') {
    ...
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ count: parseInt(response.headers['x-entities-count'] || '0', 10) }, null, 2)
      }]
    };
  } else if (method === 'get') {
    ...
    return {
      content: [{
        type: 'text',
        text: JSON.stringify({ success: true, status: response.status }, null, 2)
      }]
    };
  }
  ...
  return {
    content: [{
      type: 'text',
      text: JSON.stringify(response.data || { success: true, status: response.status }, null, 2)
    }]
  };
}
